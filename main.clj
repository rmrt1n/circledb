(declare match-part)
(defn match-variable
  [variable triple-part ctx]
  (let [bound (get ctx variable)]
    (if bound
      (match-part bound triple-part ctx)
      (assoc ctx variable triple-part))))

(defn variable?
  [p]
  (and (keyword? p) (= \? (second (str p)))))

(defn match-part
  [pattern-part tripple-part ctx]
  (when ctx
    (if (variable? pattern-part)
      (match-variable pattern-part tripple-part ctx)
      (if (= pattern-part tripple-part)
        ctx
        nil))))
...
(defn relevant-triples
  [[id attr value] db]
  (cond
    (variable? id)    (get-in db [:e id])
    (variable? attr)  (get-in db [:a attr])
    (variable? value) (get-in db [:v value])
    :else (:triples db)))
...
(defn query
  [{:keys [find where]} db]
  (let [ctxs      (query-where where db {})
        actualize (fn [ctx find]
                    (mapv
                      (fn [key]
                        (if (variable? key)
                          (get ctx key)
...
(defn index-by
  ...
  (reduce
    (fn [acc triple]
      (let [key (nth triple i)]
        (-> acc
            (assoc key (or (get acc key) []))
            (update key conj triple))
        ))
    {}
    triples))
...
(defn db
  [triples]
  {:tripples tripples
   :e (index-by triples 0)
   :a (index-by triples 1)
   :v (index-by triples 2)})

(= {:?movie/id 200 :?director/id 100}
   (match-pattern [:?movie/id :movie/director :?director/id]
...
(= [{:?movie/id 202} {:?movie/id 203} {:?movie/id 204}]
   (query-single [:?movie/id "movie/year" 1987]
                 data
                 {}))

(= [{:?movie/id 200 :?director/id 100 :?director/name "James Cameron"}]
   (query-where [[:?movie/id "movie/title" "The Terminator"]
                 [:?movie/id "movie/director" :?director/id]
                 [:?director/id "person/name" :?director/name]]
                 data
                 {}))

(= [["James Cameron"]]
   (query {:find [:?director/name]
           :where [[:?movie/id "movie/title" "The Terminator"]
                   [:?movie/id "movie/director" :?director/id]
                   [:?director/id "person/name" :?director/name]]}
          (db data)))
